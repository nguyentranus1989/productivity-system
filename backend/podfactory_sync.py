...
class PodFactorySync:
    ...
    def write_direct_to_db(self, activities_batch):
        ...
        try:
            ...
            if activities_batch:
                logger.info(f"First activity keys: {list(activities_batch[0].keys())}")
                if "metadata" in activities_batch[0]:
                    logger.info(f"Metadata keys: {list(activities_batch[0]['metadata'].keys())}")
            values = []
            for activity in activities_batch:
                ...
                values.append((
                    f"pf_{metadata.get('podfactory_id', '')}_{activity.get('timestamp', '')}",  # report_id
                    activity.get("employee_id"),
                    metadata.get("action", ""),  # activity_type from metadata
                    activity.get("scan_type", ""),  # scan_type
                    metadata.get("role_id", 3),
                    activity.get("quantity", 0),  # items_count
                    activity.get("timestamp"),  # window_start
            ...
            cursor.executemany("""
                ...
                (report_id, employee_id, activity_type, scan_type, role_id, items_count,
            ...
            conn.commit()
            logger.info(f"✅ Direct DB: Wrote {len(values)} activities")
            return True
    ...
    def sync_activities(self, use_last_sync=True):
        ...
        for activity in activities:
            ...
            if len(activities_batch) >= batch_size:
                if self.write_direct_to_db(activities_batch):
    ...
    def run_continuous_sync(self, interval_minutes=5):
        ...
        while True:
            try:
                logger.info(f"Running sync at {self.get_central_datetime()}")
                self.sync_activities(use_last_sync=True)
