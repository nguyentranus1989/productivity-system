...
class ProductivityCalculator:
    ...
    def calculate_active_time(self, activities: List[Dict], role_config = None) -> int:
        ...
        Uses PREVIOUS activity's threshold for gap calculations
        """
        if not activities:
            return 0
        ...
        # Get clocked minutes for this period
        ...
        if not clock_data or not clock_data['total_minutes']:
            # Fallback to counting activities
            return len(activities) * 10
        
        total_clocked = clock_data['total_minutes']
        
        # Calculate excess idle time using PREVIOUS activity thresholds
        total_excess_idle = 0
        prev_activity = None
        ...
        # Active time = clocked time - excess idle
