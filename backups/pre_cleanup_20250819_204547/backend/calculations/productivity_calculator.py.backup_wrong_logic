...
class ProductivityCalculator:
    ...
    def calculate_active_time(self, activities: List[Dict], role_config: RoleConfig) -> int:
        """
        Calculate active time in minutes based on role type
        
        For ALL workers: Each 10-minute window with activity = 10 minutes active
        This is because PodFactory only reports windows where work was done
        """
        if not activities:
            return 0
        
        # Count unique 10-minute windows for ALL workers
        unique_windows = set()
        for activity in activities:
            ...
            window_key = (
                window_start_central.year,
                window_start_central.month,
                window_start_central.day,
                window_start_central.hour,
                window_start_central.minute // 10
            )
            unique_windows.add(window_key)
        
        # Each window is 10 minutes of active work
        return len(unique_windows) * 10
    ...
    def process_employee_day(self, employee_id: int, process_date: date = None) -> Dict:
        ...
        try:
            ...
            # Use first activity's role for active time calculation
            if activities:
    ...
    def detect_idle_periods(self, employee_id: int, activities: List[Dict], 
        ...
        for i in range(1, len(activity_timeline)):
            prev_end = activity_timeline[i-1]['end']
            curr_start = activity_timeline[i]['start']
            
            gap_minutes = (curr_start - prev_end).total_seconds() / 60
            
            if gap_minutes > role_config.idle_threshold_minutes:
                # Record idle period
                idle_period = {
                    'employee_id': employee_id,
                    'start_time': prev_end,
                    ...
                    'threshold_minutes': role_config.idle_threshold_minutes
                }
