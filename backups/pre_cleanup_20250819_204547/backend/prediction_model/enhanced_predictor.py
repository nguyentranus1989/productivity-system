...
class EnhancedWarehousePredictor:
    ...
    def calculate_qc_capacity(self):
        ...
        if top_days:
            ...
            max_capacity = np.mean([safe_float(d['daily_qc']) for d in top_days if d['daily_qc'] is not None])
            practical_capacity = float(max_capacity * 0.95)  # 95% of max
        else:
            practical_capacity = 2000.0  # Default fallback
    ...
    def detect_customer_patterns(self, target_date):
        ...
        if result and self.base_average > 0:
            multiplier = safe_float(result['avg_orders']) / self.base_average if self.base_average else 1
    ...
    def analyze_recent_performance(self):
        ...
        for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']:
            ...
            if day_data:
                ...
                errors = [(safe_float(d['actual_orders']) / safe_float(d['predicted_orders'])) 
                         for d in day_data if d['predicted_orders'] > 0]
                if errors:
                    avg_error = np.mean(errors)
                    if avg_error > 1.3:
                        adjustments[day] = 1.15
                        print(f"  🚨 Auto-adjusting {day}: +15% (significantly under-predicting)")
                    elif avg_error > 1.2:
                        adjustments[day] = 1.10
                        print(f"  📈 Auto-adjusting {day}: +10% (under-predicting)")
                    elif avg_error > 1.1:  # Consistently under-predicting
                        adjustments[day] = 1.05  # Boost by 5%
                        print(f"  📈 Auto-adjusting {day}: +5% (was under-predicting)")
                    elif avg_error < 0.7:
                        adjustments[day] = 0.85
                        print(f"  🚨 Auto-adjusting {day}: -15% (significantly over-predicting)")
                    elif avg_error < 0.8:
                        adjustments[day] = 0.90
                        print(f"  📉 Auto-adjusting {day}: -10% (over-predicting)")
    ...
    def train_enhanced(self):
        ...
        self.base_average = safe_float(baseline['avg']) if baseline['avg'] else 0
        self.std_deviation = safe_float(baseline['std_dev']) if baseline['std_dev'] else 0
        ...
        for row in cursor.fetchall():
            self.patterns['day_of_week'][row['day']] = {
                'multiplier': safe_float(row['avg_orders']) / self.base_average if self.base_average else 1,
                'average': safe_float(row['avg_orders']) if row['avg_orders'] else 0,
                'std_dev': safe_float(row['std_dev']) if row['std_dev'] else 0
            }
