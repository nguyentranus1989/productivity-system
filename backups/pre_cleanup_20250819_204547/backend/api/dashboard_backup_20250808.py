...
def get_cost_analysis():
    ...
    try:
        ...
        WITH employee_hours AS (
            ...
            SELECT 
                ...
                CASE 
                    ...
                    ELSE ep.pay_rate 
                END as hourly_rate,
                SUM(ct.total_minutes / 60.0) as clocked_hours,
        ...
        employee_activities AS (
            -- Calculate active hours based on expected rates for the date range
            SELECT 
                al.employee_id,
                SUM(al.items_count / rc.expected_per_hour) as active_hours,
                SUM(al.items_count) as items_processed,
                COUNT(DISTINCT DATE(al.window_start)) as active_days
            FROM activity_logs al
            INNER JOIN role_configs rc ON rc.id = al.role_id
            INNER JOIN employee_hours eh ON eh.id = al.employee_id 
                AND al.window_start >= eh.first_clock_in
            WHERE DATE(CONVERT_TZ(al.window_start, '+00:00', 'America/Chicago')) BETWEEN %s AND %s
            AND al.source = 'podfactory'
        ...
        SELECT 
            ...
            eh.hourly_rate,
            eh.days_worked,
            ROUND(eh.clocked_hours, 2) as clocked_hours,
            ROUND(LEAST(eh.clocked_hours, COALESCE(ea.active_hours, 0)), 2) as active_hours,
            ROUND(GREATEST(0, eh.clocked_hours - LEAST(eh.clocked_hours, COALESCE(ea.active_hours, 0))), 2) as non_active_hours,
            ROUND(LEAST(100, COALESCE(ea.active_hours, 0) / NULLIF(eh.clocked_hours, 0) * 100), 1) as utilization_rate,
            ROUND(
                CASE 
            ...
            ROUND(LEAST(eh.clocked_hours, COALESCE(ea.active_hours, 0)) * eh.hourly_rate, 2) as active_cost,
            ROUND(GREATEST(0, eh.clocked_hours - LEAST(eh.clocked_hours, COALESCE(ea.active_hours, 0))) * eh.hourly_rate, 2) as non_active_cost,
        ...
        SELECT 
            ...
            ROUND(SUM(
                ...
                CASE 
                    WHEN ep.pay_type = 'salary' THEN ep.pay_rate / 22 / 8
                    ELSE ep.pay_rate
                END
            ), 2) as total_cost
        FROM activity_logs al
        ...
        totals = {
            ...
            'total_clocked_hours': sum(float(emp.get('clocked_hours', 0)) for emp in employee_costs),
            ...
            'total_labor_cost': sum(float(emp['total_cost']) for emp in employee_costs),
            'total_active_cost': sum(float(emp.get('active_cost', 0)) for emp in employee_costs),
            'total_non_active_cost': sum(float(emp.get('non_active_cost', 0)) for emp in employee_costs),
            'total_items': sum(emp['items_processed'] for emp in employee_costs),
            'total_days': len(set([d for emp in employee_costs for d in range(emp.get('days_worked', 0))])),
            'avg_hourly_rate': sum(float(emp['hourly_rate']) for emp in employee_costs) / len(employee_costs) if employee_costs else 0,
        }
